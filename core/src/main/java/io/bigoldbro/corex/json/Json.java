...
import io.bigoldbro.corex.exception.EncodeException;
...
public class Json {
    ...
    static {
        ...
        // custom types
        module.addSerializer(JsonObjectImpl.class, new JsonObjectSerializer());
        module.addSerializer(JsonArrayImpl.class, new JsonArraySerializer());
    ...
    static Object checkAndCopy(Object val, boolean copy) {
        if (val == null) {
            ...
            if (copy) {
                val = (new JsonObjectImpl((Map) val)).copy();
            } else {
                val = new JsonObjectImpl((Map) val);
            }
        } else if (val instanceof List) {
            if (copy) {
                val = (new JsonArrayImpl((List) val)).copy();
            } else {
    ...
    private static class JsonObjectSerializer extends JsonSerializer<JsonObjectImpl> {
        @Override
        public void serialize(JsonObjectImpl value, JsonGenerator jgen, SerializerProvider provider) throws IOException {
            jgen.writeObject(value.getMap());
        }
    }

    private static class JsonArraySerializer extends JsonSerializer<JsonArrayImpl> {
        @Override
        public void serialize(JsonArrayImpl value, JsonGenerator jgen, SerializerProvider provider) throws IOException {
